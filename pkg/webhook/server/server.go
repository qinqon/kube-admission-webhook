...
func (s *Server) Start(stop <-chan struct{}) error {
	...
	certManager := certificate.NewManager(s.mgr.GetClient(), s.webhookName, s.webhookType)
	...
	err = certManager.WaitForReadiness()
	if err != nil {
		return errors.Wrap(err, "failed waitting for cert manager readiness")
	}
